...
/// Extract the package name and version from one line of
/// `cargo tree --prefix none --format '{p}'` output.
///
/// Such a line reads `name vX.Y.Z`, possibly followed by annotations in
/// parentheses: the path of a local package, `(proc-macro)`, and `(*)` for
/// a subtree that was already printed. Returns `None` for anything else.
pub fn parse_cargo_tree_line(line: &str) -> Option<(&str, &str)> {
...
mod tests {
    ...
    // ----- parse_cargo_tree_line -----

    #[test]
    fn parses_registry_crate_line() {
        assert_eq!(parse_cargo_tree_line("serde v1.0.219"), Some(("serde", "1.0.219")));
    }

    #[test]
    fn parses_annotated_lines() {
        assert_eq!(
            parse_cargo_tree_line("gix-macros v0.1.9 (/home/me/gitoxide/gix-macros) (proc-macro)"),
            Some(("gix-macros", "0.1.9"))
        );
        assert_eq!(parse_cargo_tree_line("bstr v1.12.3 (*)"), Some(("bstr", "1.12.3")));
        assert_eq!(
            parse_cargo_tree_line("gix-path v0.10.21 (C:\\src\\gitoxide\\gix-path) (*)"),
            Some(("gix-path", "0.10.21"))
        );
    }
    ...
    fn keeps_prerelease_and_build_metadata() {
        assert_eq!(
            parse_cargo_tree_line("foo v1.0.0-beta.1+build.7"),
            Some(("foo", "1.0.0-beta.1+build.7"))
        );
    }
    ...
    #[test]
    fn rejects_lines_without_a_version() {
        assert_eq!(parse_cargo_tree_line(""), None);
        assert_eq!(parse_cargo_tree_line("serde"), None);
        assert_eq!(parse_cargo_tree_line("serde 1.0.219"), None);
        assert_eq!(parse_cargo_tree_line("warning: something"), None);
    }
}
