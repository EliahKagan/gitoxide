...
The `gix` and `ein` binaries embed license, copyright, attribution, and notice
information for every third-party crate statically linked into them. The same
information ships alongside the binaries in release archives as
`THIRD-PARTY-LICENSES.txt` (pre-rendered) and `THIRD-PARTY-LICENSES.json`
(structured).

At runtime, use either binary's `licenses` subcommand to query this data:

```console
$ gix licenses                   # summary table of every third-party crate
$ gix licenses <crate>           # full attribution for a single crate
$ gix licenses --all             # same as THIRD-PARTY-LICENSES.txt
$ gix licenses --format json     # machine-readable manifest

$ ein licenses …                 # identical surface on `ein`
```

The embedded manifest is regenerated by `build.rs` on every build, so users
building from source (`cargo build`, `cargo install`) get the same coverage
as the release binaries — no external tooling required.
